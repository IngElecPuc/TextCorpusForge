...
from textforge.io.readers_txt import iter_text_lines, load_text_lines
from textforge.io.readers_xml import iter_ces_alignment, parse_ces_alignment
...
def build_dataset_records(dataset_cfg: dict, dataset_root: str | Path, limits: dict[str, Any] | None = None) -> tuple[list[SegmentRecord], list[ParallelPairRecord], list[DocumentRecord]]:
...
def _build_from_ces_alignment(dataset: dict, dataset_root: Path, limits: dict[str, Any] | None = None) -> tuple[list[SegmentRecord], list[ParallelPairRecord], list[DocumentRecord]]:
    ...
    rows = list(iter_ces_alignment(alignment_path, max_pairs=max_pairs if max_pairs > 0 else None))
    if not rows:
        return [], [], []
    needed_src = {key for row in rows for key in row['src_keys'] if _is_numericish(key)}
    needed_tgt = {key for row in rows for key in row['tgt_keys'] if _is_numericish(key)}
    src_max = max((int(float(k.replace(':', '.').split('.')[0])) for k in needed_src), default=0)
    tgt_max = max((int(float(k.replace(':', '.').split('.')[0])) for k in needed_tgt), default=0)
    src_index = {str(idx): (seq, raw, norm) for seq, (idx, raw, norm) in enumerate(_iter_until_line(src_path, src_max), start=1)}
    tgt_index = {str(idx): (seq, raw, norm) for seq, (idx, raw, norm) in enumerate(_iter_until_line(tgt_path, tgt_max), start=1)}
    ...
    for group_name, group_rows in group_buckets.items():
        ...
        for row in group_rows:
            ...
            for key in row['src_keys']:
                lookup = _lookup_key(src_index, key)
            ...
            for key in row['tgt_keys']:
                lookup = _lookup_key(tgt_index, key)
                seq, raw, norm = lookup or (None, '', '')
            ...
            src_text_raw = ' '.join((_lookup_key(src_index, k) or (None, '', ''))[1] for k in row['src_keys']).strip()
            tgt_text_raw = ' '.join((_lookup_key(tgt_index, k) or (None, '', ''))[1] for k in row['tgt_keys']).strip()
            quality_flags = []
            ...
            pairs.append(ParallelPairRecord(
                ...
                pair_id=stable_hash(f"{dataset['name']}:{doc_group_id}:pair:{pair_count}"), pair_index=pair_count,
...
def _build_from_ids_sidecar(dataset: dict, dataset_root: Path, limits: dict[str, Any] | None = None) -> tuple[list[SegmentRecord], list[ParallelPairRecord], list[DocumentRecord]]:
    src_path = _require(resolve_in_root(dataset_root, dataset['files']['src_text']))
    tgt_path = _require(resolve_in_root(dataset_root, dataset['files']['tgt_text']))
    ...
    for idx_row in iter_ids_rows(ids_path):
        ...
        if max_pairs > 0 and len(idx_rows) >= max_pairs:
            break
    if not idx_rows:
        return [], [], []
    src_max = len(idx_rows)
    tgt_max = len(idx_rows)
    src_lines = list(_iter_until_line(src_path, src_max))
    tgt_lines = list(_iter_until_line(tgt_path, tgt_max))
    segments: list[SegmentRecord] = []
    pairs: list[ParallelPairRecord] = []
    documents: list[DocumentRecord] = []
    ...
    for idx_row, src_row, tgt_row in zip(idx_rows, src_lines, tgt_lines):
...
def _build_from_monotonic_plain(dataset: dict, dataset_root: Path, limits: dict[str, Any] | None = None) -> tuple[list[SegmentRecord], list[ParallelPairRecord], list[DocumentRecord]]:
    src_path = _require(resolve_in_root(dataset_root, dataset['files']['src_text']))
    tgt_path = None
    ...
    return build_monotonic_parallel_records(
...
def _build_from_plain_stream(dataset: dict, dataset_root: Path, limits: dict[str, Any] | None = None) -> tuple[list[SegmentRecord], list[ParallelPairRecord], list[DocumentRecord]]:
    src_path = _require(resolve_in_root(dataset_root, dataset['files']['src_text']))
    tgt_path = None
    candidates = dataset['files'].get('tgt_text_candidates')
    if candidates:
    ...
    return build_unaligned_parallel_streams(dataset_root, dataset['name'], dataset['domain'], dataset['src_lang'], dataset['tgt_lang'], src_path, tgt_path, group_name, is_dialogue=is_dialogue, max_segments=int((limits or {}).get('max_segments', 0) or 0))


def _iter_until_line(path: Path, max_line_number: int):
    if max_line_number <= 0:
        return []
    rows = []
    for idx, raw, norm in iter_text_lines(path):
        rows.append((idx, raw, norm))
        if idx >= max_line_number:
            break
    return rows
