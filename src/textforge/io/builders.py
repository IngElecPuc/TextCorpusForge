...
from textforge.io.readers_parallel_text import build_unaligned_parallel_streams
...
def build_dataset_records(dataset_cfg: dict, dataset_root: str | Path) -> tuple[list[SegmentRecord], list[ParallelPairRecord], list[DocumentRecord]]:
    dataset_root = Path(dataset_root)
    dataset = dataset_cfg['dataset']
    reader = dataset['reader']
    if reader == 'ces_alignment_xml':
        return _build_from_ces_alignment(dataset, dataset_root)
    if reader == 'ids_sidecar':
        return _build_from_ids_sidecar(dataset, dataset_root)
    if reader == 'parallel_sidecar_plain':
        return _build_from_plain_stream(dataset, dataset_root)
    if reader == 'tmx':
        return build_tmx_records(dataset_root, dataset['name'], dataset['domain'], dataset['src_lang'], dataset['tgt_lang'])
    if reader == 'tsv_parallel':
        return build_tsv_records(dataset_root, dataset['name'], dataset['domain'], dataset['src_lang'], dataset['tgt_lang'])
    raise ValueError(f"Reader no soportado: {reader}")


def _build_from_ces_alignment(dataset: dict, dataset_root: Path) -> tuple[list[SegmentRecord], list[ParallelPairRecord], list[DocumentRecord]]:
    ...
    src_index = {str(idx): (seq, raw, norm) for seq, (idx, raw, norm) in enumerate(src_lines, start=1)}
    tgt_index = {str(idx): (seq, raw, norm) for seq, (idx, raw, norm) in enumerate(tgt_lines, start=1)}

    alignments = parse_ces_alignment(alignment_path)
    ...
    for group_name, rows in group_buckets.items():
        ...
        for row in rows:
            ...
            for key in row['src_keys']:
                seq, raw, norm = src_index.get(key, (None, '', ''))
            ...
            for key in row['tgt_keys']:
                seq, raw, norm = tgt_index.get(key, (None, '', ''))
            ...
            src_text_raw = ' '.join(src_index.get(k, (None, '', ''))[1] for k in row['src_keys']).strip()
            tgt_text_raw = ' '.join(tgt_index.get(k, (None, '', ''))[1] for k in row['tgt_keys']).strip()
            quality_flags = []
            ...
            pairs.append(
                ParallelPairRecord(
                    ...
                    origin_path=str(alignment_path),
                    doc_group_id=doc_group_id,
                    pair_id=stable_hash(f"{dataset['name']}:{doc_group_id}:pair:{pair_count}"),
                    ...
                    src_segment_keys=src_keys,
                    tgt_segment_keys=tgt_keys,
                    src_sequence_indices=src_seq_indices,
                    tgt_sequence_indices=tgt_seq_indices,
                    alignment_type='ces_explicit',
                    ...
                    metadata={'from_doc': rows[0].get('from_doc', ''), 'to_doc': rows[0].get('to_doc', '')},
                )
            )
            pair_count += 1
...
def _build_from_ids_sidecar(dataset: dict, dataset_root: Path) -> tuple[list[SegmentRecord], list[ParallelPairRecord], list[DocumentRecord]]:
    src_path = _require(resolve_in_root(dataset_root, dataset['files']['src_text']))
    tgt_path = _require(resolve_in_root(dataset_root, dataset['files']['tgt_text']))
    ...
    for idx_row, src_row, tgt_row in zip(iter_ids_rows(ids_path), src_lines, tgt_lines):
        group_name = f"{idx_row['src_doc']}|{idx_row['tgt_doc']}" if dataset.get('structural', {}).get('group_from_alignment_docs', True) else 'global'
        doc_group_id = stable_hash(f"{dataset['name']}:{group_name}")

        src_line_idx, src_raw, src_norm = src_row
        tgt_line_idx, tgt_raw, tgt_norm = tgt_row
        ...
        pairs.append(
            ParallelPairRecord(
                dataset_name=dataset['name'],
                domain=dataset['domain'],
                origin_path=str(ids_path),
                doc_group_id=doc_group_id,
                pair_id=stable_hash(f"{dataset['name']}:{doc_group_id}:pair:{pair_index}"),
                ...
                src_segment_keys=idx_row['src_keys'] or [str(src_line_idx)],
                tgt_segment_keys=idx_row['tgt_keys'] or [str(tgt_line_idx)],
                src_sequence_indices=[src_line_idx],
                tgt_sequence_indices=[tgt_line_idx],
                alignment_type='ids_explicit',
                src_words=len(src_norm.split()),
                tgt_words=len(tgt_norm.split()),
                length_ratio=_length_ratio(src_norm, tgt_norm),
...
def _build_from_plain_stream(dataset: dict, dataset_root: Path) -> tuple[list[SegmentRecord], list[ParallelPairRecord], list[DocumentRecord]]:
    src_path = _require(resolve_in_root(dataset_root, dataset['files']['src_text']))
    tgt_path = None
    ...
    return build_unaligned_parallel_streams(dataset_root, dataset['name'], dataset['domain'], dataset['src_lang'], dataset['tgt_lang'], src_path, tgt_path, group_name, is_dialogue=is_dialogue)
...
def _segment_from_line(dataset: dict, origin_path: str, doc_group_id: str, lang: str, segment_key: str, sequence_index: int | None, raw: str, norm: str) -> SegmentRecord:
    ...
    return SegmentRecord(
        ...
        doc_group_id=doc_group_id,
        segment_id=stable_hash(f"{dataset['name']}:{doc_group_id}:{lang}:{segment_key}"),
        lang=lang,
        text_raw=raw,
        text_norm=norm,
        sequence_index=int(sequence_index or 0),
        segment_key=segment_key,
        segment_key_numeric=_maybe_int(segment_key),
        is_dialogue=bool(dataset.get('structural', {}).get('is_dialogue', False)),
...
def _document_summary(dataset: dict, doc_group_id: str, origin_path: str, segments: list[SegmentRecord], pairs: list[ParallelPairRecord]) -> DocumentRecord:
    ...
    return DocumentRecord(
        ...
        tgt_lang=tgt_lang,
        is_parallel=True,
        segment_count_src=sum(1 for s in doc_segments if s.lang == src_lang),
        segment_count_tgt=sum(1 for s in doc_segments if s.lang == tgt_lang),
        pair_count=len(doc_pairs),
        total_chars_src=sum(s.num_chars for s in doc_segments if s.lang == src_lang),
        total_chars_tgt=sum(s.num_chars for s in doc_segments if s.lang == tgt_lang),
        total_words_src=sum(s.num_words for s in doc_segments if s.lang == src_lang),
        total_words_tgt=sum(s.num_words for s in doc_segments if s.lang == tgt_lang),
    )
...
def _require(path: Path | None) -> Path:
    if path is None:
        raise FileNotFoundError('No se encontró un archivo requerido en la raíz del dataset.')
    return path
